...
# ── Python toolchain ──
curl -LsSf https://astral.sh/uv/install.sh | sh   # or: pip install uv==0.11.29
uv python install 3.12
uv init --package ba-tools --python 3.12
...
**If tier = `light` (default golden path):**
...
**If tier = `standard` (optional BPMN / Mermaid export):**
...
**If tier = `strict` (publishable client bundle):**
...
**If team mode enabled (BRD-023–025):**
...
**If Windows BA laptop (pilot primary):**
...
**If Linux CI headless (conformance corpus):**
...
└──────────────────────────┬──────────────────────────────────┘
                           │
┌──────────────────────────▼──────────────────────────────────┐
│  Skill Layer — Cursor Agent Skills (.cursor/skills/ba-*)    │
│  SKILL.md + workflows → calls ba-tools, owns judgment       │
└──────────────────────────┬──────────────────────────────────┘
                           │ subprocess (JSON stdout/stderr)
┌──────────────────────────▼──────────────────────────────────┐
│  ba-tools CLI (Python 3.12 + Typer)                         │
│  jsonschema gates │ hashlib │ filelock │ Jinja2 render      │
└───────┬─────────────────────────────┬───────────────────────┘
        │                             │
        ▼                             ▼
┌───────────────┐            ┌────────────────────┐
│  .ba-ops/     │            │  Local Renderers    │
│  file-state   │            │  mmdc (Node 22)     │
│  Git-tracked  │            │  draw.io Desktop    │
└───────────────┘            └────────────────────┘
```
...
---
*Stack research for: ba-daily-ops harness-first BA workflow*
*Researched: 2026-07-20*
